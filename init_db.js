...
setTimeout(() => {
    // Seed Products
    const productStmt = db.prepare("INSERT INTO products (name, category, price, image, description, stock, unit) VALUES (?, ?, ?, ?, ?, ?, ?)");
    products.forEach(p => {
        productStmt.run(p.name, p.category, p.price, p.image, p.description, p.stock, p.unit);
    });
    productStmt.finalize();
    console.log("Products seeded");

    // Seed Categories
    const catStmt = db.prepare("INSERT INTO categories (name, icon) VALUES (?, ?)");
    categories.forEach(c => {
        catStmt.run(c.name, c.icon);
    });
    catStmt.finalize();
    console.log("Categories seeded");

    // Seed Users
    const userStmt = db.prepare("INSERT INTO users (name, email, password, address) VALUES (?, ?, ?, ?)");
    users.forEach(u => {
        userStmt.run(u.name, u.email, u.password, u.address);
    });
    userStmt.finalize();
